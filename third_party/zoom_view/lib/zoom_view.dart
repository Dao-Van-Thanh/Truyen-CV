...
class _ZoomViewState extends State<ZoomView> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.parentPageController != null) {
      _parentPageTouchHandler = _TouchHandler(
        controller: widget.parentPageController!,
      );
    }
  ...
  void didUpdateWidget(ZoomView oldWidget) {
    ...
    if (widget.parentPageController != oldWidget.parentPageController) {
      _parentPageTouchHandler = widget.parentPageController == null
          ? null
          : _TouchHandler(controller: widget.parentPageController!);
    }
  }
  ...
  _TouchHandler? _parentPageTouchHandler;
  _TouchHandler? _activeVerticalTouchHandler;
  ...
  bool get _canDelegateToParentPage {
    return widget.scrollAxis == Axis.vertical &&
        widget.parentPageController != null &&
        (_scale - 1.0).abs() < 0.001;
  }
  ...
  void _startPanDrag({
    ...
    _activeVerticalTouchHandler = _verticalTouchHandler;
    if (_canDelegateToParentPage) {
      final position = _verticalController.position;
      if (position.minScrollExtent == 0 && position.maxScrollExtent == 0) {
        _activeVerticalTouchHandler = _parentPageTouchHandler;
      }
    }

    _activeVerticalTouchHandler?.handleDragStart(dragDetails);
    _horizontalTouchHandler.handleDragStart(dragDetails);
  }

  void _maybeTransferVerticalDragToParent(ScaleUpdateDetails details) {
    if (!_canDelegateToParentPage || _activeVerticalTouchHandler != _verticalTouchHandler) {
      return;
    }

    final position = _verticalController.position;
    final delta = details.focalPointDelta.dy * _scale;
    final reachedBottom = delta < 0 && position.pixels >= position.maxScrollExtent;
    final reachedTop = delta > 0 && position.pixels <= position.minScrollExtent;

    if (!reachedTop && !reachedBottom) return;

    _verticalTouchHandler.handleDragCancel();
    _activeVerticalTouchHandler = _parentPageTouchHandler;
    _activeVerticalTouchHandler?.handleDragStart(
      DragStartDetails(
        globalPosition: details.focalPoint * _scale,
          ...
          child: GestureDetector(
            ...
            onScaleUpdate: (ScaleUpdateDetails details) {
              ...
              switch (_dragMode) {
                ...
                case DragMode.pan:
                  _maybeTransferVerticalDragToParent(details);
                  ...
                  _activeVerticalTouchHandler?.handleDragUpdate(verticalDetails);
            ...
            onScaleEnd: (ScaleEndDetails details) {
              ...
              _activeVerticalTouchHandler?.handleDragEnd(endDetails);
              _activeVerticalTouchHandler = null;
