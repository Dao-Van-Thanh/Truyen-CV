...
extension NullableResponseExtension on Response? {
  ApiResult<T> parseDataNovel<T>(T Function(Map<String, dynamic>) fromJson) {
    try {
      if (this == null) {
  ...
  ApiResult<T> parseDataComic<T>(T Function(Map<String, dynamic>) fromJson) {
    try {
      if (this == null) {
        return ApiResult.error(ErrorResponseModel.defaultError());
      }
      ...
      final isError =
          jsonData['status'] == false || jsonData['status'] != 'success';
      final parsed = fromJson(jsonData);
      if (isError) {
        final err = ErrorResponseModel.fromJson(jsonData);
        // Trả về cả data lẫn error
